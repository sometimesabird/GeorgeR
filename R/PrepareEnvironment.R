...
PrepareEnvironment <- function(
    rmarkdown, # TRUE if file is an Rmarkdown document, FALSE if it is a regular script
    fig.dims = NULL, # for Rmarkdown you can pass a vector of (width, height) for default size of figures in a chunk
    update = FALSE, # check for updates automatically
    quiet = FALSE # suppress update messages
) {
  ...
  if (rmarkdown) {
    ...
    # Check if meta package is available before using
