...
CheckAndUpdatePackage <- function(quiet = FALSE) {
  ...
  tryCatch({
    ...
    if (new_version > current_version && !quiet) {
      ...
      # Try to reload the package without restarting R
      tryCatch({
        if (requireNamespace("pkgload", quietly = TRUE)) {
          # Use pkgload for hot reloading
          pkgload::load_all(pkgload::inst("GeorgeR"), quiet = TRUE)
          message("Package reloaded successfully - no restart needed!")
        } else {
          # Fallback to detach/library method
          detach("package:GeorgeR", unload = TRUE)
          library(GeorgeR, warn.conflicts = FALSE)
          message("Package reloaded successfully - no restart needed!")
        }
      }, error = function(e) {
        message("Automatic reload failed - restart R session to use the new version")
      })
    } else if (!quiet) {
